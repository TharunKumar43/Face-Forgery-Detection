...
def validate_image(image):
    """Validate image meets requirements for processing"""
    if image is None:
        return False
...
def convert_image(image):
    """Convert image to standard RGB format"""
...
class validation_dataset(Dataset):
    ...
    def frame_extract(self, path):
        ...
        while success:
            ...
            # Debug print to check frame properties
            print(f"Frame shape: {image.shape}, dtype: {image.dtype}, min: {image.min()}, max: {image.max()}")
            
            # Ensure image is in uint8 format with proper range
            if image.dtype != np.uint8:
                image = image.astype(np.uint8)
            ...
            if len(image.shape) != 3 or image.shape[2] != 3:
                print(f"Skipping invalid frame shape: {image.shape}")
                continue
...
def predict_page(request):
    if request.method == "GET":
        ...
        for i in range(sequence_length):
            ...
            # Skip if frame is empty or None
            if frame is None or frame.size == 0:
                print(f"Empty frame at index {i}")
                continue
            ...
            try:
                if len(frame.shape) == 2:  # Grayscale
                    ...
                    if frame.shape[2] == 4:  # RGBA
                        ...
                        print(f"Unexpected channels at frame {i}: {frame.shape}")
                        continue
                else:
                    print(f"Invalid frame dimensions at {i}: {frame.shape}")
                    continue
                    
                # Additional validation
                if rgb_frame.min() < 0 or rgb_frame.max() > 255:
                    print(f"Invalid pixel values at frame {i}")
                    continue
                ...
                try:
                    ...
                    print(f"PIL save error at frame {i}: {str(e)}")
                    continue
